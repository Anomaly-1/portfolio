...
type ColorMode = 'original' | 'grayscale' | 'duotone' | 'custom' | 'colorized-grayscale';
...
interface DitherShaderProps {
  ...
  /** Color quantization levels for original mode (higher = more detail, default 4) */
  colorLevels?: number;
  /** Color for colorized-grayscale mode (black to this color based on brightness) */
...
export const DitherShader: React.FC<DitherShaderProps> = ({
  ...
  const parsedMonochromeColor = parseColor(monochromeColor);
      ...
      for (let y = 0; y < displayHeight; y += effectivePixelSize) {
        for (let x = 0; x < displayWidth; x += effectivePixelSize) {
          ...
          switch (colorMode) {
            ...
            case 'colorized-grayscale': {
              // Use luminance to blend from black to the monochrome color
              const brightness = clamp(luminance, 0, 1);
              outputColor = [
                Math.round(parsedMonochromeColor[0] * brightness),
                Math.round(parsedMonochromeColor[1] * brightness),
                Math.round(parsedMonochromeColor[2] * brightness),
              ];
              break;
            }
            case 'duotone': {
            ...
            default: {
              ...
              outputColor = [
                Math.round(adjustedR / (255 / colorLevels)) * (255 / colorLevels),
                Math.round(adjustedG / (255 / colorLevels)) * (255 / colorLevels),
                Math.round(adjustedB / (255 / colorLevels)) * (255 / colorLevels),
              ];
              break;
            }
          }
